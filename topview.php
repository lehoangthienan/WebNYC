			...
			<div class="box_center_main">
				...
				<div class="row">
					<?php
						...
						$query="SELECT * FROM tblsanpham WHERE ten LIKE '%{$tukhoa}%' AND status='0' ORDER BY view DESC LIMIT {$start},{$limit}";
						...
						if($per_page > 1)
						{
							...
							if($current_page !=1)
							{
								echo "<li><a href='topview.php?s=".($start-$limit)."&p={$per_page}'>Back</a></li>";
							}
							...
							{ 
								if($i!=$current_page)
								{
									echo "<li><a href='topview.php?s=".($limit*($i-1))."&p={$per_page}'>{$i}</a></li>";
								}
								else
								{
							...
							{
								echo "<li><a href='topview.php?s=".($start+$limit)."&p={$per_page}'>Next</a></li>";
							}
						}
						echo "</ul>";
					?>
