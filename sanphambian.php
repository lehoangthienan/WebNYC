				...
				<div class="row">
					<?php
						...
						else
						{
							...
							$query_pg="SELECT COUNT(id) FROM tblsanpham WHERE ten LIKE '%{$tukhoa}%'  AND status='1'";
						...
						$query="SELECT * FROM tblsanpham WHERE ten LIKE '%{$tukhoa}%' AND status='1' ORDER BY id DESC LIMIT {$start},{$limit}";
						$results=mysqli_query($dbc,$query);
						kt_query($results,$query);
						while($sanpham=mysqli_fetch_array($results,MYSQLI_ASSOC))
						{
						?>
						<a href="hienthisp.php?id=<?php echo $sanpham['id'];?>">
						...
						if($per_page > 1)
						{
							...
							if($current_page !=1)
							{
								echo "<li><a href='sanphambian.php?s=".($start-$limit)."&p={$per_page}'>Back</a></li>";
							}
							...
							{ 
								if($i!=$current_page)
								{
									echo "<li><a href='sanphambian.php?s=".($limit*($i-1))."&p={$per_page}'>{$i}</a></li>";
								}
								else
								{
							...
							{
								echo "<li><a href='sanphambian.php?s=".($start+$limit)."&p={$per_page}'>Next</a></li>";
							}
						}
						echo "</ul>";
					?>
